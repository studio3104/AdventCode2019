...
public class Day03 {
    ...
    private class Position {
        ...
        Position getNext(char direction) {
            switch (direction) {
                default:
    ...
    class Wire {
        ...
        Wire(List<String> directions) {
            this.directions = directions;
        }

        Set<Position> getTracks() {
            Position current = new Position(0, 0);
            Set<Position> tracks = new HashSet<>();

            for (String d: directions) {
                char direction = d.toCharArray()[0];
                int stepCount = Integer.parseInt(d.substring(1));
    ...
    int getClosestManhattanDistance(Wire wire1, Wire wire2) {
        Set<Position> tracks1 = wire1.getTracks();
        Set<Position> tracks2 = wire2.getTracks();

        Set<Position> intersection = new HashSet<>(tracks1);
        Object result = intersection.retainAll(tracks2);

        return intersection
                .stream()
                .map(s -> Math.abs(s.x) + Math.abs(s.y))
                .mapToInt(v -> v)
                .min()
                .orElse(0);
    }
